...
low_res = im_names_hub[0]   #Could also let this be a hubble image...if so, see the hubble loop below for how to setup grabbing the data, header, pixel conversion from the hdu
hdu1 = fits.open(low_res)
hdu1_data = hdu1[1].data
hdu1_header = hdu1[1].header
...
for name in [im_names_hub[1]]:
    ...
    try: 
        ...
        hdu1_conv_scaled = hdu1_conv[0].data
        ...
        para = False
        array, footprint = reproject_exact(conv2_path, hdu1_header, parallel=para)
        file_start = 'Regridded_Hub/regrid_'
        regrid_hub_path = fits_saver(array, wcs_header, '164_hub.fits', file_start)
        regrid_foot_path = fits_saver(footprint, wcs_header, '164_foot.fits', file_start)
        ...
        array[array < 0.05 * np.max(array)] = 0
        array[array < 0] = 0
        ...
        w = WCS(hdu1_header)
        wcs_header = w.to_header()
        save_path = 'extinctionmap/regrid_'  #See fits_saver's "save_path" description for explanation
...
#More detailed example/explanations of the steps for an ind4ividual case
