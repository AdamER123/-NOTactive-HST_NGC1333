...
from photutils.detection import DAOStarFinder #needed instead of threshold to detect stars (as opposed to galaxies)
...
class DashData(object):
    ...
    def create_seg_map(self):
        ...
        UPDATE 4_27_22: Basing source detection off DAOPHOT (https://photutils.readthedocs.io/en/stable/detection.html)
        Parameters
        ----------
        self : object
            DashData object created from an individual IMA file.
        ...
        sources.rename_column('xcentroid', 'x_mean')
        sources.rename_column('ycentroid', 'y_mean')
        # tbl = cat.to_table()
        ...
        #saving the sources from segmentation map
        # hdu = fits.PrimaryHDU(segm.data)
        hdu = fits.PrimaryHDU(make_gaussian_sources_image(data.shape, sources))
        ...
        ascii.write(sources, 'segmentation_maps/{}_source_list.dat'.format(self.root))
    ...
    def diff_seg_map(self, cat_images=None, remove_column_names=True, nsigma=1.0, sig=6.0, npixels=5):
        ...
        for index, diff in enumerate(input_images, start=1):
            ...
            ascii.write(sources, 'segmentation_maps/{}_{:02d}_diff_source_list.dat'.format(self.root, index))
