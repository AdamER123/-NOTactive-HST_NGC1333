...
if switch1 == 'on':
    ...
    res_str = 'no_halpha' #used to label what we're saving, usually related to units or whether we're doing a gaussian or box convolution, etc
...
if switch2 == 'on':
    ...
    R_H = 33.9 # 17.5 24.5, an alternate try that was abandoned
...
#         sum_sys = np.sqrt(np.sum((obs-theory)**2./obs**2. for (obs,theory) in zip(pix_flux, equ_sys)))
        ...
        #for method curve_fit...func for non-linear equation...
    def scatt_func(lam_arr, A_V_fit, f_H, f_Fe):
        return np.array([
...
#         return equ_sys
    ...
    cont_flux2 = np.array([flux_cutout[3], flux_cutout[4], flux_cutout[5]])
...
#     xin_nk = [3.11706881e-30, 5., 2.79259524e-17, 4.48608665e-17]   #np.array([1.5, 4.7, 5e-17, 5e-17])
    xin2 = [5., 8.17922089e-18, 3.68824803e-18]
    ...
    Tend = 400.
    T_increment = 50
    bb_guess = [Tstart, 5e-29]
    res_str_fit = 'no656_RFe'+str(R_Fe).replace('.', 'p')+'RH'+str(R_H).replace('.', 'p')+'Tstart'+str(Tstart)+'C'+str(5e-29)+'fluxlim0p1e-17' #'C5_lt0_perc-0_start6000_end2000_inc400_AV5_f5E-18_ratio2_3_withOI'
    
    #beginning loop...
    for i in range(cont_flux2[0].shape[0]):
        for j in range(cont_flux2[0].shape[1]):
            pix_flux = np.array([cont_flux2[0][i, j], cont_flux2[1][i, j], cont_flux2[2][i, j]])
            xin2 = [5., pix_flux[1], pix_flux[2]]#8.17922089e-18, 3.68824803e-18]
    #         sigma_arr = [cont_flux[0][i, j]/1e5, cont_flux[1][i, j]/1e5, cont_flux[2][i, j]/1e5, cont_flux[3][i, j]/1e5]
            ...
            try:
                ...
                #trying Dan's method of setting temperature...need to fit a blackbody for a set of fluxes...
                C, T = curve_fit(B_lam, lam_arr, pix_flux, p0=bb_guess)
...
#                     C_fit, A_V_fit, f_H_fit, f_Fe_fit = solved
                ...
                #can then be used to 
                obs_intens = pix_flux
                theory_intens = scatt_func(lam_arr, C, A_V_fit, f_H_fit, f_Fe_fit)
                n_obs = len(obs_intens)
                rms = np.sqrt(np.sum((obs_intens - theory_intens)**2./(obs_intens)**2.)) 
                f_H_obspredic = (R_H * f_H_fit + B_lam(lam_arr[0], T, C)) * y_arr[0]**A_V_fit
                
                #saving results
                fit_test.append([i, j, rms, theory_intens, T, C, A_V_fit, f_H_fit, f_Fe_fit])
...
#                 print('no fit found')
                fit_arr2[count] = [i, j, np.nan, [np.nan, np.nan, np.nan], np.nan, np.nan, np.nan, np.nan, np.nan]
...
file_start_extinc = nonlin_folder + '/nohalpha_' + res_str + '_deextinc_'
